(function($) {
    ...
    $('#btn-generate-vouchers').on('click', function() {
        ...
        if (count < 1 || count > 1000) {
            showAdminToast('Количество должно быть от 1 до 1000', 'error');
            return;
        }

        var amount = parseFloat($('#voucher_amount').val());
        if (!amount || amount <= 0) {
            showAdminToast('Укажите сумму скидки', 'error');
            return;
        }
        ...
        $(this).prop('disabled', true).html(
            '<span class="dashicons dashicons-update spin" style="vertical-align:text-bottom;margin-right:4px"></span> Создание...'
        );
    ...
    function generateBatch(offset, total, batchId) {
        $.ajax({
            url: wcVoucher.ajaxUrl,
            type: 'POST',
            timeout: 30000,
            data: {
                action: 'wc_voucher_generate_batch',
                nonce: wcVoucher.nonce,
                count: total,
                offset: offset,
                batch_id: batchId,
                prefix: $('#voucher_prefix').val(),
                code_length: $('#voucher_code_length').val(),
                discount_type: $('#voucher_discount_type').val(),
                amount: $('#voucher_amount').val(),
                expiry: $('#voucher_expiry').val(),
                usage_limit: $('#voucher_usage_limit').val(),
            ...
            success: function(response) {
                if (!response.success) {
                    showAdminToast(response.data || 'Неизвестная ошибка', 'error');
                    resetGenerator();
                    return;
                }
                ...
                if (data.done) {
                    ...
                    generateBatch(data.offset, total, batchId);
                }
            },
            error: function(xhr) {
                var msg = 'Ошибка сервера';
                if (xhr.status === 0) msg = 'Нет связи с сервером';
                else if (xhr.status === 504) msg = 'Таймаут — попробуйте меньше ваучеров';
    ...
    function showResult() {
        ...
        setTimeout(function() {
            ...
            resetGenerator();
            showAdminToast('Создано ' + allCodes.length + ' ваучеров!', 'success');
        }, 500);
    }

    function resetGenerator() {
        isGenerating = false;
        $('#btn-generate-vouchers').prop('disabled', false).html(
            '<span class="dashicons dashicons-tickets-alt" style="vertical-align:text-bottom;margin-right:4px"></span> Создать ваучеры'
        );
    }
    ...
    $('#btn-download-csv').on('click', function(e) {
        ...
        showAdminToast('CSV файл скачан', 'success');
    });

    // === Copy to clipboard ===
    $(document).on('click', '#btn-copy-codes', function() {
        var textarea = document.getElementById('voucher-result-codes');
        if (!textarea || !textarea.value) return;

        if (navigator.clipboard) {
            navigator.clipboard.writeText(textarea.value).then(function() {
                showAdminToast('Коды скопированы в буфер обмена', 'success');
            });
        } else {
            textarea.select();
            document.execCommand('copy');
            showAdminToast('Коды скопированы', 'success');
        }
    });
    ...
    function showAdminToast(message, type) {
        ...
        var toast = $(
            '<div class="voucher-admin-toast" style="' +
            ...
            'max-width:400px;animation:vToastIn .4s cubic-bezier(.34,1.56,.64,1)">' +
        ...
        if (!$('#voucher-toast-keyframes').length) {
            $('head').append(
                '<style id="voucher-toast-keyframes">' +
                '@keyframes vToastIn{0%{opacity:0;transform:translateX(60px)}100%{opacity:1;transform:translateX(0)}}' +
