...
class WC_Voucher_Tracker {
    ...
    public function track_usage_on_order($order_id) {
        ...
        if (!$order) return;
        if ($order->get_meta('_voucher_usage_tracked') === 'yes') return;
        ...
        foreach ($coupons as $code) {
            ...
            update_post_meta($coupon->get_id(), '_voucher_last_used_by', $email);
            update_post_meta($coupon->get_id(), '_voucher_last_used_order_id', $order_id);
            update_post_meta($coupon->get_id(), '_voucher_last_used_customer', $name);

            $history = get_post_meta($coupon->get_id(), '_voucher_usage_history', true);
            if (!is_array($history)) $history = [];
            $history[] = ['order_id' => $order_id, 'email' => $email, 'customer' => $name, 'date' => current_time('mysql'), 'discount' => $order->get_discount_total()];
    ...
    public function add_order_metabox() {
        $singular = WC_Voucher_Settings::get('name_plural');
        /* translators: %s: plural voucher name */
        $title = sprintf(__('Used %s', 'wc-voucher-manager'), mb_strtolower($singular));

        add_meta_box('wc_voucher_order_info', $title, [$this, 'render_order_metabox'], 'shop_order', 'side', 'default');
        $screen = function_exists('wc_get_page_screen_id') ? wc_get_page_screen_id('shop-order') : null;
        if ($screen) add_meta_box('wc_voucher_order_info', $title, [$this, 'render_order_metabox'], $screen, 'side', 'default');
    }

    public function render_order_metabox($post_or_order) {
        $order = ($post_or_order instanceof WP_Post) ? wc_get_order($post_or_order->ID) : $post_or_order;
        if (!$order) { echo '<p>' . esc_html__('Order not found', 'wc-voucher-manager') . '</p>'; return; }

        $coupons = $order->get_coupon_codes();
        if (empty($coupons)) {
            $plural_lc = mb_strtolower(WC_Voucher_Settings::get('name_plural'));
            printf('<p style="color:#999">%s</p>', sprintf(esc_html__('No %s used', 'wc-voucher-manager'), esc_html($plural_lc)));
            return;
        }
        ...
        foreach ($coupons as $code) {
            ...
            if ($coupon->get_id()) {
                ...
                printf('<li><a href="%s"><strong>%s</strong></a> &mdash; %s</li>', esc_url(admin_url('post.php?post=' . $coupon->get_id() . '&action=edit')), esc_html(strtoupper($code)), $label);
            } else {
                printf('<li><strong>%s</strong> <em>(%s)</em></li>', esc_html(strtoupper($code)), esc_html__('deleted', 'wc-voucher-manager'));
            }
        }
        echo '</ul>';
    }
    ...
    public function add_order_column($columns) {
        ...
        foreach ($columns as $key => $label) {
            $new[$key] = $label;
            if ($key === 'order_total') $new['voucher_used'] = WC_Voucher_Settings::get('name_singular');
        }
        return $new;
    }
    ...
    private function output_voucher_column($order) {
        if (!$order) { echo '&mdash;'; return; }
        $coupons = $order->get_coupon_codes();
        if (empty($coupons)) { echo '&mdash;'; return; }
        echo implode(' ', array_map(function($code) {
